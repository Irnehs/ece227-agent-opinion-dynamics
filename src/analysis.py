...
def compute_statistics(df: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    results["mean_by_style_timestep"] = (
        df.groupby(["timestep", "communication_style"])["ranking"].mean().unstack()
    )

    results["median_by_style_timestep"] = (
        df.groupby(["timestep", "communication_style"])["ranking"].median().unstack()
    )

    results["mode_by_style_timestep"] = (
        df.groupby(["timestep", "communication_style"])["ranking"]
        .apply(lambda x: x.mode().iloc[0] if len(x.mode()) > 0 else np.nan)
        .unstack()
    )

    results["std_by_style_timestep"] = (
        df.groupby(["timestep", "communication_style"])["ranking"].std().unstack()
    )
...
def compute_convergence_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for (param_name, param_value, trial_id), group in df.groupby(
        ...
        initial_range = (
            group[group["timestep"] == 0]["ranking"].max()
            - group[group["timestep"] == 0]["ranking"].min()
        )
        final_range = (
            group[group["timestep"] == final_timestep]["ranking"].max()
...
def compute_opinion_trajectory(df: pd.DataFrame) -> pd.DataFrame:
    trajectory = (
        df.groupby(["param_value", "timestep"])
        .agg({"ranking": ["mean", "std", "min", "max", "median"]})
        .reset_index()
    )
...
def compute_style_trajectory(df: pd.DataFrame) -> pd.DataFrame:
    trajectory = (
        df.groupby(["param_value", "timestep", "communication_style"])
        .agg({"ranking": ["mean", "std", "count"]})
        .reset_index()
    )
...
def compute_delta_statistics(df: pd.DataFrame) -> pd.DataFrame:
    delta_stats = (
        df.groupby(["param_value", "timestep", "communication_style"])
        .agg({"delta_ranking": ["mean", "std", "min", "max"]})
        .reset_index()
    )
...
def compute_agreement_distribution(df: pd.DataFrame) -> pd.DataFrame:
    dist = (
        df.groupby(["param_value", "timestep", "ranking"])
        .size()
...
def aggregate_across_trials(df: pd.DataFrame) -> pd.DataFrame:
    agg = (
        df.groupby(["param_name", "param_value", "timestep", "communication_style"])
        .agg({"ranking": ["mean", "std", "median"], "delta_ranking": ["mean", "std"]})
        .reset_index()
    )
...
def simulate_degroot(
    initial_opinions: np.ndarray, adjacency_matrix: np.ndarray, timesteps: int
) -> np.ndarray:
    """
...
def compute_degroot_comparison(
    df: pd.DataFrame, adjacency_matrices: dict[tuple, np.ndarray] | None = None
) -> pd.DataFrame:
    """
    ...
    for (param_name, param_value, trial_id), group in df.groupby(
        ...
        if adjacency_matrices and (param_value, trial_id) in adjacency_matrices:
            ...
            degroot_trajectory = simulate_degroot(
                initial_opinions, adj_matrix, int(final_timestep)
            )
...
def compute_resilience_metrics(df: pd.DataFrame, bot_opinion: int = 10) -> pd.DataFrame:
    """
    ...
    for group_key, group in df.groupby(groupby_cols):
        ...
        adoption_rate = (
            adoption_count / len(final_rankings) if len(final_rankings) > 0 else 0.0
        )
...
def aggregate_resilience_by_topology(resilience_df: pd.DataFrame) -> pd.DataFrame:
    ...
    agg = (
        resilience_df.groupby(["param_name", "param_value"])
        .agg(
            {
    ...
    agg.columns = [
        ...
        "mean_drift",
        "std_drift",
        "mean_polarization_change",
        "std_polarization_change",
    ]
...
def run_full_analysis(
    ...
    if include_resilience or ("is_bot" in df.columns and df["is_bot"].any()):
        ...
        results["resilience_by_topology"] = aggregate_resilience_by_topology(
            results["resilience"]
        )
    ...
    if output_dir:
        ...
        results["convergence"].to_csv(
            output_dir / "convergence_metrics.csv", index=False
        )
        results["opinion_trajectory"].to_csv(
            output_dir / "opinion_trajectory.csv", index=False
        )
        results["style_trajectory"].to_csv(
            output_dir / "style_trajectory.csv", index=False
        )
        results["polarization"].to_csv(
            output_dir / "polarization_metrics.csv", index=False
        )
        results["distribution"].to_csv(
            output_dir / "agreement_distribution.csv", index=False
        )
        results["trial_aggregated"].to_csv(
            output_dir / "trial_aggregated.csv", index=False
        )
        results["degroot_comparison"].to_csv(
            output_dir / "degroot_comparison.csv", index=False
        )

        if "resilience" in results:
            results["resilience"].to_csv(
                output_dir / "resilience_metrics.csv", index=False
            )
            results["resilience_by_topology"].to_csv(
                output_dir / "resilience_by_topology.csv", index=False
            )
...
if __name__ == "__main__":
    ...
    if results_dir.exists():
        ...
        if len(df) > 0:
            ...
            print(
                "No records found"
                + (f" for experiment '{args.experiment}'" if args.experiment else "")
            )
    else:
