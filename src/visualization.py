...
def plot_agreement_distribution_over_time(
    ...
    if n_styles == 0:
        print(
            f"No communication styles found for param_value={param_value}, skipping plot"
        )
        return None
    ...
    axes[0].set_title(
        f"Agreement Distribution Over Time (p={param_value})"
        if param_value
        else "Agreement Distribution Over Time"
    )
...
def plot_convergence_comparison(
    ...
    agg = (
        convergence_df.groupby("param_value")
        .agg({"variance_reduction": ["mean", "std"], "final_std": ["mean", "std"]})
        .reset_index()
    )
    ...
    ax.axhline(
        y=1, color="gray", linestyle="--", alpha=0.5, label="Convergence threshold"
    )
    ax.legend()
...
def plot_ranking_histogram(
    ...
    for ax, t in zip(axes, timesteps):
        ...
        for style in sorted(t_df["communication_style"].unique()):
            ...
            offset = (
                style_keys.index(style) * 0.15 - 0.225 if style in style_keys else 0
            )
            ax.bar(
                bins[:-1] + 0.5 + offset,
                counts,
...
def plot_parameter_sweep_summary(
    ...
    ax = axes[0, 1]
    variance_by_param = (
        df.groupby(["param_value", "timestep"])["ranking"].var().reset_index()
    )
    for param_value in sorted(df["param_value"].unique()):
    ...
    for style in sorted(df["communication_style"].unique()):
        s_df = df[df["communication_style"] == style]
        final_by_param = (
            s_df[s_df["timestep"] == final_t].groupby("param_value")["ranking"].mean()
        )
        color = STYLE_COLORS.get(style, None)
        ax.plot(
            final_by_param.index,
            final_by_param.values,
            label=style,
            marker="o",
            linewidth=2,
            color=color,
...
def plot_degroot_comparison(
    ...
    ax = axes[0]
    agg = (
        degroot_df.groupby("param_value")
        .agg(
            {
    ...
    ax.errorbar(
        x,
        agg[("degroot_predicted_mean", "mean")],
        yerr=agg[("degroot_predicted_mean", "std")],
        marker="s",
        capsize=5,
        linewidth=2,
        linestyle="--",
        label="DeGroot Prediction",
    )
    ax.set_xlabel("Parameter Value")
    ...
    agg_var = (
        degroot_df.groupby("param_value")
        .agg(
            {"llm_final_var": ["mean", "std"], "degroot_predicted_var": ["mean", "std"]}
        )
        .reset_index()
    )
    ...
    ax.errorbar(
        agg_var["param_value"],
        agg_var[("degroot_predicted_var", "mean")],
    ...
    ax = axes[2]
    deviation_agg = (
        degroot_df.groupby("param_value")["mean_deviation"]
        .agg(["mean", "std"])
        .reset_index()
    )
    ax.bar(
        deviation_agg["param_value"],
        deviation_agg["mean"],
        yerr=deviation_agg["std"],
        capsize=5,
        alpha=0.7,
        color=COLORS[2],
    )
...
def plot_resilience_by_topology(
    ...
    ax = axes[0, 0]
    agg = (
        resilience_df.groupby("param_value")["resistance_index"]
        .agg(["mean", "std"])
        .reset_index()
    )
    ax.bar(
        agg["param_value"].astype(str),
        agg["mean"],
        yerr=agg["std"],
        capsize=5,
        alpha=0.8,
    ...
    ax.set_ylim(0, 1.1)
    ax.axhline(
        y=0.5, color="gray", linestyle="--", alpha=0.5, label="Neutral threshold"
    )

    ax = axes[0, 1]
    agg = (
        resilience_df.groupby("param_value")["adoption_rate"]
        .agg(["mean", "std"])
        .reset_index()
    )
    ax.bar(
    ...
    ax = axes[1, 0]
    agg = (
        resilience_df.groupby("param_value")["drift_toward_bot"]
        .agg(["mean", "std"])
        .reset_index()
    )
    ax.bar(
    ...
    ax = axes[1, 1]
    agg = (
        resilience_df.groupby("param_value")["polarization_change"]
        .agg(["mean", "std"])
        .reset_index()
    )
...
def generate_all_plots(
    ...
    plot_mean_opinion_trajectory(
        df,
        group_by="communication_style",
        output_path=output_dir / "trajectory_by_style.png",
    )
    plt.close()

    plot_mean_opinion_trajectory(
        df, group_by="param_value", output_path=output_dir / "trajectory_by_param.png"
    )
    plt.close()

    plot_ranking_histogram(df, output_path=output_dir / "ranking_histogram.png")
    plt.close()

    plot_parameter_sweep_summary(
        df, output_path=output_dir / "parameter_sweep_summary.png"
    )
    plt.close()
    ...
    if convergence_df is not None and len(convergence_df) > 0:
        plot_convergence_comparison(
            convergence_df, output_path=output_dir / "convergence_comparison.png"
        )
        plt.close()

    if degroot_df is not None and len(degroot_df) > 0:
        plot_degroot_comparison(
            degroot_df, output_path=output_dir / "degroot_comparison.png"
        )
        plt.close()

    if resilience_df is not None and len(resilience_df) > 0:
        plot_resilience_by_topology(
            resilience_df, output_path=output_dir / "resilience_by_topology.png"
        )
        plt.close()
    ...
    if results_dir.exists():
        ...
        if len(df) > 0:
            ...
            if "is_bot" in df.columns and df["is_bot"].any():
                ...
                print("Computing resilience metrics for bot experiments")
            ...
            try:
                ...
                print(
                    "Skipping semantic plots, continuing with other visualizations..."
                )
